#!/usr/bin/env bash
# ==============================================================================
# K.I.T.T. Ecosystem Installer for Linux
# ==============================================================================
set -euo pipefail
...
echo " | . \  _| |_   | |     | |   "
echo " |_|\_\|_____|  |_|     |_|   "
echo "  Ecosystem Installer (Linux) "
echo -e "${NC}"
...
check_cmd() {
    if ! command -v "$1" &>/dev/null; then
        log_err "Missing required command: $1. Please install it before proceeding."
        return 1
    fi
}
...
if command -v espeak-ng &>/dev/null || command -v espeak &>/dev/null || command -v spd-say &>/dev/null; then
    log_ok "System TTS synthesis backend found."
else
    log_warn "No system TTS found (espeak-ng/speech-dispatcher). Voice output will be disabled until installed."
fi

# 3. Create Configuration Directories with Strict Permissions
CONFIG_ROOT="${XDG_CONFIG_HOME:-$HOME/.config}/kitt"
...
echo -e "${BOLD}${GREEN}        K.I.T.T. Ecosystem Installed Successfully!             ${NC}"
echo -e "${BOLD}${GREEN}================================================================${NC}"
echo ""
echo -e "${BOLD}Next Steps:${NC}"
echo ""
echo -e " 1. ${BOLD}Start KITT Daemon & Control Center:${NC}"
echo -e "    $ROOT_DIR/kitt-assistant/target/release/kittd"
echo ""
echo -e " 2. ${BOLD}Open KITT Control Center Web GUI:${NC}"
...
echo -e "    kitt"
echo ""
echo -e " 4. ${BOLD}Interact via CLI hotkey utility:${NC}"
